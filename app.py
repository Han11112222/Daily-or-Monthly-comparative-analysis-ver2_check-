...
- 마지막에 **일별비율 합계가 1이 되도록 정규화(raw / SUM(raw))**
    ...
    # 1. [기존/AS-IS] 원래 색상 (파랑/빨강/초록)
    # [중요] marker_color 지정을 삭제하여 기존 코드 로직(Plotly Default)을 그대로 따르게 함
    # (형님의 원래 코드에 marker_color가 없었다면 자동 할당 색상 사용, 있었다면 그 색상 유지)
    # 여기서는 형님의 코드 스타일대로 w1, w2, we를 나누어 그리되, 색상은 지정하지 않음 (자동 파랑/빨강/초록 계열)
    w1_df = view[view["구분"] == "평일1(월·금)"].copy()
    w2_df = view[view["구분"] == "평일2(화·수·목)"].copy()
    wend_df = view[view["구분"] == "주말/공휴일"].copy()

    # 색상 지정 삭제 (형님의 원래 코드 상태로 복구)
