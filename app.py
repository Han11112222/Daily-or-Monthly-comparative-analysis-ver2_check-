...
def tab_daily_plan(df_daily: pd.DataFrame):
    ...
    # ★ 우측 상단 배치: Title Row에 Columns 사용
    ...
    diff_mj = 0.0
    if use_calibration:
        # 활성화 시 설정 패널을 그래프 바로 위에 Expander로 열어줌
...
- 마지막에 **일별비율 합계가 1이 되도록 정규화(raw / SUM(raw))**
    ...
    # 1. [기존/AS-IS] 원래 색상 (파랑/빨강/초록 - 형님 원래 코드 방식 그대로)
    # 색상 지정을 하지 않음으로써 Plotly 기본 색상(파랑, 빨강, 초록) 사용
    ...
    * 상단 우측 **'이상치 보정 활성화'**를 체크하면 보정된 값(회색)을 비교할 수 있습니다.
    """)
