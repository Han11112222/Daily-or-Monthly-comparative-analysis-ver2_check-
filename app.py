...
def tab_daily_plan(df_daily: pd.DataFrame):
    ...
    # ★★★ [수정 1] 그래프 자리 먼저 만들기 (placeholder) ★★★
    # 그래프를 먼저 보여주고, 그 아래에 버튼/제어패널이 오도록 배치
    chart_placeholder = st.empty()
    
    # ─────────────── [UI 하단] 보정 로직 및 버튼 ───────────────
    # 그래프 바로 아래에 2단 컬럼 (제목 + 버튼)
    c_head, c_ctrl = st.columns([4, 1]) # [수정] 제목(4) : 버튼(1) 비율
    
    with c_head:
        st.markdown("#### 📊 2. 일별 예상 공급량 & Outlier 분석")
    
    with c_ctrl:
        # [수정] 그래프 우측 상단(시각적으로는 그래프 하단 제목줄 우측)에 버튼 배치
        use_calib = st.checkbox("✅ 이상치 보정 활성화", value=False)
        
    diff_mj = 0
    mask_out = pd.Series([False] * len(view)) # 초기화
        ...
        with st.expander("🛠️ 보정 구간 및 재배분 설정", expanded=True):
            ...
            if isinstance(d_out, tuple) and len(d_out) == 2 and isinstance(d_fix, tuple) and len(d_fix) == 2:
                ...
                if mask_out.any():
                    ...
                    # 상한/하한 체크 (형님의 기존 로직엔 없었으나 보정 기능 강화)
                    view.loc[mask_out, "보정_예상공급량(MJ)"] = np.where(
                        view.loc[mask_out, "보정_예상공급량(MJ)"] > view.loc[mask_out, "Bound_Upper"],
                        view.loc[mask_out, "Bound_Upper"],
                        np.where(
                            view.loc[mask_out, "보정_예상공급량(MJ)"] < view.loc[mask_out, "Bound_Lower"],
                            view.loc[mask_out, "Bound_Lower"],
                            view.loc[mask_out, "보정_예상공급량(MJ)"]
                        )
                    )
    ...
    # 1. [기존 그래프] (녹색 제거, 주말=빨강, 평일=파랑)
    w1 = view[view["구분"] == "평일1(월·금)"].copy()
    w2 = view[view["구분"] == "평일2(화·수·목)"].copy()
    we = view[view["구분"] == "주말/공휴일"].copy()

    # Opacity: 보정 활성화 시 기존 막대는 흐리게 처리하지 않고 그대로 둠 (형님 요청: 덮어씌우기)
    # 다만 보정값(회색)이 올라가면 겹쳐 보이게 됨.
    
    # [수정] 색상: 평일1=진한파랑, 평일2=연한파랑, 주말=빨강
    ...
    fig.add_trace(go.Bar(x=we["일"], y=we["예상공급량(GJ)"], name="주말/공휴일", marker_color="#EF553B", width=0.8)) # 녹색(#00CC96) -> 빨강

    # 2. [보정 그래프] (TO-BE) - ★값이 변경된 날짜만 회색★
    if use_calib:
        # 변경된 날짜만 필터링 (부동소수점 오차 고려하여 1 미만 차이는 무시)
    ...
    # 테이블 및 나머지 UI (순서: 1.분배기준 -> 2.월별 -> 3.일별)
    st.divider()

    st.markdown("### 🧩 1. 일별 공급량 분배 기준") # 순서 1
    st.markdown(
        """
...
- 마지막에 **일별비율 합계가 1이 되도록 정규화(raw / SUM(raw))**
    ...
    st.markdown("#### 📌 2. 월별 계획량(1~12월) & 연간 총량") # 순서 2
    ...
    st.markdown("#### 📋 3. 일별 비율, 예상 공급량 테이블") # 순서 3
