...
def tab_daily_plan(df_daily: pd.DataFrame):
    ...
    # 1. [기존 그래프] 디폴트 색상 유지 (녹색 방지용 색상 지정 X - 원래 코드대로)
    # 형님의 디폴트 코드는 marker_color를 지정하지 않아 Plotly Default(Blue, Red, Green..)를 사용함.
    # 형님 요청: "색깔 건들지 마" -> marker_color 속성 제거하여 순정 상태로 둠.
