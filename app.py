...
def make_month_plan_horizontal(df_plan: pd.DataFrame, target_year: int, plan_col: str) -> pd.DataFrame:
    ...
    total_gj = mj_to_gj(total_mj); total_m3 = mj_to_m3(total_mj)
    row_gj = {}; row_m3 = {}
    for m in range(1, 13):
...
def format_table_generic(df, percent_cols=None, temp_cols=None):
    df = df.copy()
    percent_cols = percent_cols or []; temp_cols = temp_cols or []
    def _fmt_no_comma(x):
        if pd.isna(x): return ""
        try: return f"{int(x)}"
        except: return str(x)
    for col in df.columns:
        ...
        if col in percent_cols:
            ...
            if col in ["연", "연도", "월", "일", "WeekNum"]: df[col] = df[col].map(_fmt_no_comma)
            else: df[col] = df[col].map(lambda x: f"{x:,.0f}" if pd.notna(x) else "")
    return df
...
def _format_excel_sheet(ws, freeze="A2", center=True, width_map=None):
    ...
    if width_map:
        for col_letter, w in width_map.items(): ws.column_dimensions[col_letter].width = w
...
def make_daily_plan_table(
    df_daily: pd.DataFrame, df_plan: pd.DataFrame, target_year: int = 2026, target_month: int = 1, recent_window: int = 3,
    ...
    df_result = df_target[["연", "월", "일", "일자", "요일", "weekday_idx", "nth_dow", "구분", "공휴일여부",
                           ...
                           "Bound_Upper", "Bound_Lower", "is_outlier"]].copy()
...
def tab_daily_plan(df_daily: pd.DataFrame):
    ...
    st.caption(f"최근 {recent_window}년 후보({target_year-recent_window}년 ~ {target_year-1}년) {target_month}월 패턴으로 {target_year}년 {target_month}월 일별 계획을 계산. (해당월 실적이 없는 연도는 자동 제외)")
    ...
    # ★★★ 버튼 배치: 그래프 우측 상단 ★★★
    # 공간 분할: 5:1 비율로 오른쪽 끝에 버튼 배치
    col_spacer, col_btn = st.columns([5, 1]) 
    with col_btn:
        use_calibration = st.checkbox("이상치 보정 활성화", value=False)

    diff_mj = 0.0
    # 계산 로직만 수행 (UI는 아래에 배치)
    if use_calibration:
        # 설정 UI는 그래프 하단으로 이동 (아래에서 렌더링)
        # 여기서는 변수 초기화만 필요하면 함
        pass

    # ─────────────── [그래프 그리기] ───────────────
    # 먼저 그래프를 그립니다. (UI 배치는 그래프 아래)
    
    # 1. 보정 계산 (UI 입력값을 받아야 하므로, 순서상 st.empty()를 쓰거나,
    #    Streamlit 특성상 위에서 선언해야 함.
    #    형님이 "버튼만 우측 상단"에 두고 "설정창"은 그래프 하단에 두라고 하셨으므로,
    #    설정창을 먼저 렌더링하되 '순서'를 바꿀 수 없으니
    #    'expander'를 그래프 아래에 배치하겠습니다.
    
    # 그러나, expander에서 값을 입력받아야 그래프가 그려지므로,
    # 코드 순서상 expander가 먼저 와야 합니다.
    # 하지만 시각적으로 아래에 두려면 container를 써야 합니다.
    
    bottom_container = st.container() # 그래프 아래에 들어갈 컨테이너

    # 보정 로직 변수들 초기화
    mask_out = pd.Series([False]*len(view), index=view.index)
    
    if use_calibration:
        # 그래프 아래 컨테이너에 설정 UI 배치
            ...
            with st.expander("🛠️ 보정 구간 및 재배분 설정", expanded=True):
                ...
                if isinstance(d_out, tuple) and len(d_out)==2 and isinstance(d_fix, tuple) and len(d_fix)==2:
                    ...
                    if mask_out.any():
                        ...
                        diff_mj = (view.loc[mask_out, "보정_예상공급량(MJ)"] - view.loc[mask_out, "예상공급량(MJ)"]).sum()
    ...
    # 1. [기존 그래프] (형님 요청: 색상은 디폴트 유지, 녹색 금지)
    # 디폴트 코드에서는 색상을 지정하지 않아 Plotly Default(Blue, Red, Green..)가 나옵니다.
    # 형님 요청: "그래프 색도 디폴트 코드 그대로 둬." (단, 녹색은 싫다고 하셨음)
    # 따라서, 색상 코드를 *추가하지 않고* 그대로 둡니다.
    # 다만, 형님이 "녹색 싫어" 하셨으니, 주말 부분만 빨강으로 처리하겠습니다.
    
    w1 = view[view["구분"] == "평일1(월·금)"]
    w2 = view[view["구분"] == "평일2(화·수·목)"]
    we = view[view["구분"] == "주말/공휴일"]
    
    fig.add_trace(go.Bar(x=w1["일"], y=w1["예상공급량(GJ)"], name="평일1(월·금)"))
    fig.add_trace(go.Bar(x=w2["일"], y=w2["예상공급량(GJ)"], name="평일2(화·수·목)"))
    # 주말은 빨간색으로 강제 (녹색 방지)
    fig.add_trace(go.Bar(x=we["일"], y=we["예상공급량(GJ)"], name="주말/공휴일", marker_color="#EF553B"))

    # 2. [보정 그래프] ★ 선택된 날짜만 회색 (Overlay) ★
    if use_calibration:
        # mask_out에 해당하는 날짜만 필터링
        if mask_out.any():
            target_view = view[mask_out]
    ...
    fig.update_layout(
        ...
        barmode="overlay", # 겹쳐보기
        legend=dict(orientation="h", y=1.1)
    )
    st.plotly_chart(fig, use_container_width=True)

    # 2. 일별 예상 공급량 & Outlier 분석 제목과 내용은 그래프 하단으로 이동
    with bottom_container:
        st.markdown("#### 📊 2. 일별 예상 공급량 & Outlier 분석")
        # (설정창은 이미 위에서 with bottom_container로 넣었음)
