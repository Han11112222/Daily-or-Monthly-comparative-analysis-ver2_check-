...
def load_daily_data():
    ...
    if not excel_path.exists():
        return pd.DataFrame(), pd.DataFrame() # 파일 없을 때 에러 방지

    df_raw = pd.read_excel(excel_path)

    # 내부 계산은 MJ 유지 (표기/다운로드는 GJ 및 m³로 변환)
    # 컬럼 존재 여부 체크
    cols = ["일자", "공급량(MJ)", "공급량(M3)", "평균기온(℃)"]
    for c in cols:
...
def load_monthly_plan() -> pd.DataFrame:
    ...
    if not excel_path.exists():
        return pd.DataFrame() # 파일 없을 때 에러 방지
...
# ─────────────────────────────────────────────
# Daily 공급량 분석용 함수 (기존 로직 유지 + Outlier 컬럼 추가)
# ─────────────────────────────────────────────
...
def tab_daily_plan(df_daily: pd.DataFrame):
    ...
    # 우측 상단 배치를 위한 컬럼 분할 (좌측: 그래프 제목, 우측: 보정 패널)
    ...
    with col_calib:
        ...
        if use_calibration:
            with st.expander("🛠️ 보정 상세 설정 (이상구간 -> 보정구간 배분)", expanded=True):
                ...
                c1, c2 = st.columns(2)
                with c1:
                    date_range_out = st.date_input("1. 보정 필요 구간 (Outlier)", value=(min_date, min_date), min_value=min_date, max_value=max_date)
                with c2:
                    date_range_dist = st.date_input("2. 보정 구간 (Redistribution)", value=(min_date, max_date), min_value=min_date, max_value=max_date)
...
- 마지막에 **일별비율 합계가 1이 되도록 정규화(raw / SUM(raw))**
    ...
    if use_calibration:
        fig.add_bar(
            ...
            marker_color="rgba(60, 60, 60, 0.6)" # 요청하신 회색 (Overlay)
        )
