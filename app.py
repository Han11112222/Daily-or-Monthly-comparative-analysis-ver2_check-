    ...
    if use_calibration:
        ...
        with st.expander("🛠️ 보정 구간 및 재배분 설정", expanded=True):
            ...
            if len(date_range_out) == 2 and len(date_range_dist) == 2:
                ...
                sum_r = view.loc[mask_dist, "일별비율"].sum()
                if mask_dist.any() and sum_r > 0:
                    view.loc[mask_dist, "보정_예상공급량(MJ)"] += diff_mj * (view.loc[mask_dist, "일별비율"] / sum_r)
...
- 마지막에 **일별비율 합계가 1이 되도록 정규화(raw / SUM(raw))**
    ...
    # 1. [기존/AS-IS] 원래 색상 유지 (녹색 삭제, 주말은 빨강)
    # 평일1: #1F77B4 (기본 파랑)
    # 평일2: #636EFA (연한 파랑)
    # 주말: #EF553B (빨강 - 기존 녹색 삭제)
    w1_df = view[view["구분"] == "평일1(월·금)"].copy()
    w2_df = view[view["구분"] == "평일2(화·수·목)"].copy()
    wend_df = view[view["구분"] == "주말/공휴일"].copy()

    fig.add_bar(x=w1_df["일"], y=w1_df["예상공급량(GJ)"], name="평일1(월·금)", marker_color="#1F77B4")
    fig.add_bar(x=w2_df["일"], y=w2_df["예상공급량(GJ)"], name="평일2(화·수·목)", marker_color="#636EFA")
    fig.add_bar(x=wend_df["일"], y=wend_df["예상공급량(GJ)"], name="주말/공휴일", marker_color="#EF553B")
    
    # 2. [보정/TO-BE] 보정된 값 (활성화 시에만 덮어씌움)
    # -> "진한 회색(투명도 60%)"으로 덮어 씌우기
    if use_calibration:
        fig.add_bar(
            x=view["일"], 
            y=view["보정_예상공급량(GJ)"], 
            name="보정 후 (Calibrated)", 
            marker_color="rgba(60, 60, 60, 0.6)" # 진한 회색 (투명도)
        )
